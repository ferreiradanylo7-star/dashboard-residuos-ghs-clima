...
with aba_lixiviacao:
    ...
    """)
with aba_resumo: # ou aba4
    st.markdown("### 📋 Resumo Executivo: Transição Normativa e Classificação de Resíduos")
    
    # Linha de destaque
    st.info("💡 **Paradigma 2024:** Foco na composição química total (perfil toxicológico) em detrimento dos ensaios lixiviados.")

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### 1. A Mudança de Eixo (2004 vs 2024)")
        st.markdown("""
        * **2004:** Baseado predominantemente em ensaios de lixiviação e solubilização.
        * **2024:** Implementação da **Lista Geral de Resíduos (LGR)** e alinhamento com o **GHS (Sistema Globalmente Harmonizado)**.
        * **Critério Técnico:** Foco em perigos intrínsecos (CBR: Carcinogenicidade, Mutagenicidade, Teratogenicidade).
        """)

    with col2:
        st.markdown("#### 2. Hierarquia (Lei 12.305/2010)")
        st.markdown("""
        1.  **Não Geração**
        2.  **Redução**
        3.  **Reutilização**
    ...
    st.markdown("#### 3. Fluxograma de Classificação (Conceitual)")
    # Infográfico em texto (Cards)
    c1, c2, c3 = st.columns(3)
    c1.metric("Passo 1", "Identificação", "Origem/Fluxo")
    c2.metric("Passo 2", "Enquadramento", "LGR (8 dígitos)")
    c3.metric("Passo 3", "Avaliação", "Perfil Toxicológico (GHS)")
    
    st.warning("⚠️ **Nota Técnica:** Substâncias com perigos HP 1 a HP 15 (baseadas na ONU/GHS) definem a periculosidade. Ensaios de lixiviação perderam o protagonismo mandatório.")

    st.markdown("---")
    st.caption("Baseado na ABNT NBR 10004-1:2024 e Política Nacional de Resíduos Sólidos (PNRS).")
